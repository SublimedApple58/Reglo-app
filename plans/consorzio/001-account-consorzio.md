...
> **AGGIORNAMENTO (2026-09-07): secondo giro di fedeltà al prototipo (4 fix di Tiziano)** — (1) notifiche campanella cambiano stato in "Guida accettata/rifiutata" con icona verde/rossa; (2) Impostazioni → "Fatturazione e pagamenti" (solo consorzio) con placeholder **Reglo × Fatture in Cloud** (partner cambiato da TeamSystem su decisione di Tiziano); (3) toggle "Visualizza per: Istruttori/Veicoli" nel dialog Visualizzazione + agenda a colonne-veicolo (il refactor columnsBy previsto come follow-up è FATTO; default resta Istruttori); (4) "Sposta" apre il dialog "Proponi un altro orario" (ghost live + controproposta persistita, campi `proposed*` su ConsorzioGuideRequest, migration `20260907161125`; conferma della scuola = fase affiliate). Dettagli in `docs/features/consorzio.md` §"Secondo giro prototipo". E2E: 23/23 regressione + 15/15 nuovi check.
> **STATO (2026-09-03): fasi 1–4 IMPLEMENTATE su `feature/consorzio`** (non ancora su staging/prod). E2E Playwright su localhost: 20/20 flussi verificati (shell, scuole, prezzi con persistenza, fatturazione con toggle saldata, richiesta→accetta con conflitti, regressione autoscuola normale). Doc definitiva: `docs/features/consorzio.md`.
> **Emendamenti in corso d'opera**: (1) prezzo guida calcolato LAZY in Fatturazione + snapshot congelato al primo toggle saldata/fatturata — NIENTE hook sui punti di creazione appuntamento (zero rischio motore, tariffe retro-applicabili fino a certificazione); (2) hamburger della shell INVARIATO (il prototipo lo tiene identico, niente voci nascoste); (3) richieste pending non renderizzate come blocchi statici in agenda (senza istruttore non hanno colonna): vivono via campanella → card + ghost nella colonna dell'istruttore scelto; (4) istruttore obbligatorio all'Accetta (picker nella card); (5) "Sposta" = click su un altro slot della griglia (stessa UX degli altri flussi di creazione).
