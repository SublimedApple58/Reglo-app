...
- `CompanyService.limits.followCarMotoEnabled` — **single global toggle** for "auto al seguito": when on, EVERY moto guida (any category AM/A1/A2/A) additionally reserves a follow car. **Since 2026-07-03 the rule is a suggestion, not a bind, on owner/instructor SINGLE-guide flows**: web create wizard + EditAppointmentDialog + mobile BookingForm + manage-lesson-vehicles all offer an explicit "Nessuna auto al seguito" option (sentinel `__none__` client-side → `followVehicleId: null`); create still demands an *explicit* choice (car or Nessuna). Student self-booking keeps the auto-assign behavior; **group moto lessons keep the hard requirement** (`validateMotoGroupSetup`). Back-compat: the legacy per-category map `followCarRules` is still read — `readFollowCarMotoEnabled` returns true if any moto category was enabled — and `parseFollowCarRulesFromLimits` now DERIVES the per-category map from the global flag (all-moto-on / off), so every existing call site keeps working.
...
- **Vehicle-change conflict check (2026-07-06, incidente Yaris Robatto 03/07)**: `updateAutoscuolaAppointmentDetails` ora blocca il cambio veicolo/auto al seguito/moto extra se uno dei veicoli AGGIUNTI è già impegnato nell'orario della guida (`findVehicleReservationConflict`: altri appuntamenti via `vehicleId` + join, più container guide di gruppo incl. flotta e follow condivisa; `code: "VEHICLE_UNAVAILABLE"`). Solo i veicoli aggiunti sono verificati (un overlap pre-esistente su un veicolo non toccato non blocca modifiche non correlate) e le guide già passate restano modificabili (record fix). Prima il cambio veicolo non aveva NESSUN check di sovrapposizione — era il buco da cui il titolare poteva doppio-prenotare un'auto (la creazione l'avrebbe bloccato).
- **Group-container check sui create staff (2026-07-06)**: `createAutoscuolaAppointment` + batch ora verificano anche i CONTAINER delle guide di gruppo (istruttore, veicolo primario, follow condivisa, flotta) — una guida di gruppo a 0 partecipanti non ha righe appuntamento ed era invisibile al conflict check staff (il flusso allievo la vedeva già via `group-lesson-busy`).
- **Moto-only enforcement BE (2026-07-07)**: `extraMotoVehicleIds` + `followVehicleId` sono consentiti SOLO quando il veicolo primario (finale) è una moto, su tutti e 3 i percorsi: `createAutoscuolaAppointment` (che prima non li validava affatto — nemmeno ownership/active), `createAutoscuolaAppointmentBatch` e `updateAutoscuolaAppointmentDetails`. Forniti esplicitamente su una guida auto → errore; ereditati cambiando il primario da moto ad auto → droppati in silenzio dal reconcile (il passaggio moto→auto pulisce follow + extra da solo). Le UI li nascondevano già, ma il BE non imponeva nulla.
- **Ghost-extra sul cambio veicolo (fix 2026-07-07, "due auto sulle guide" a Robatto)**: in `updateAutoscuolaAppointmentDetails` il branch che preserva le moto extra quando NON tocchi `extraMotoVehicleIds` filtrava dalle righe `role="primary"` esistenti solo il NUOVO primario — cambiando veicolo, il VECCHIO restava agganciato come "extra" fantasma (mobile mostrava "Auto1 + Auto2", il web solo il primario). Ora esclude anche `appointment.vehicleId` (il vecchio). Bonifica prod 2026-07-07: cancellate 12 righe fantasma (auto non-moto ≠ primario, 11 Robatto + 1 Macchiavello) dopo verifica che nessun primario restante fosse occupato nel suo orario.
- Follow-car is now wired end-to-end:
  ...
  - **Edit**: `EditAppointmentDialog` edits the primary vehicle, follow car and extra motos on existing lessons; `updateAutoscuolaAppointmentDetails` accepts `followVehicleId` + `extraMotoVehicleIds` and reconciles the join rows transactionally (helper `reconcileAppointmentVehicles`, which also fixed a latent stale-join bug when changing the primary). **Picker filtering (2026-06-30, mirrors the mobile manage-lesson sheet)**: all three pickers are limited to vehicles the lesson's **instructor can use** (`instructorCanUseVehicle` from `lib/autoscuole/group-moto`); the **primary** vehicle also requires **student-license** eligibility (`vehicleServesLicense`, moto hierarchy); **extra motos** follow the **same student-license hierarchy** as the primary (equal-or-lower moto category — an AM student can't add A1/A2 motos; fixed 2026-07-01, previously unfiltered) on top of instructor-usability; the **follow car** is a B car the instructor can use. Each moto option/chip shows its **license category**. The currently-assigned vehicle always stays selectable. Needs `poolInstructorIds` (now surfaced on the agenda-bootstrap vehicles) + the student's `licenseCategory`/`transmission` (passed into the dialog). The **create** flow (mode-first, `AutoscuoleAgendaPage`) also filters primary + extra motos by student eligibility (moto hierarchy) but does **not** filter by instructor yet — symmetric with the mobile `BookingForm`.
...
> **Perché la gerarchia rimorchio esiste (REG-507, 2026-09-22).** Un rimorchio non è
> un veicolo guidabile: niente motore, niente posto di guida, nessuna disponibilità
> propria. Un camion **è** un veicolo C e **diventa** un autotreno quando gli si
> aggancia un rimorchio — quindi il parco mezzi registra motrici, e una CE deve poter
> girare sul camion che la scuola possiede davvero. Fino al 22/09/2026 non poteva:
> BE/C/CE/D/DE erano state aggiunte (commit `6c83277`) dentro il ramo scritto per la
> B ("B combacia solo con B"), ereditandone la regola senza che nessun commit
> spiegasse perché, e il commento nello schema Prisma descriveva ancora un mondo di
> sole `B | AM | A1 | A2 | A`. In produzione il consorzio aveva 2 allievi CE e 0
