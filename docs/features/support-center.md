...
- `sendSupportMessage({ body })` — upsert thread + crea messaggio + `unreadForAdmin++` in transazione; poi `after()` → email di avviso a `SUPPORT_NOTIFY_EMAILS` (= `GLOBAL_ADMIN_EMAIL` + gabriele.ruzzu@reglo.it + support@reglo.it, dal 2026-08-03) via `sendDynamicEmail` (no-op su staging). Anche le richieste del dialog Novità (`submitNewsFeedback`) vanno a `SUPPORT_NOTIFY_EMAILS`; il feedback prodotto ("Feedback N★") resta solo a `GLOBAL_ADMIN_EMAIL`.
