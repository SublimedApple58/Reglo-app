...
- **UI**: `components/pages/Consorzio/` — `ConsorzioSchoolsPage`, `ConsorzioSchoolDetailPage` (route `autoscuole/scuole/[schoolId]`), `ConsorzioBillingPage`, `ConsorzioPrezziPane` (sub-tab "Prezzi" in `BookingsTab.tsx`, al posto di "Crediti e prezzi" che per il consorzio è nascosto)
