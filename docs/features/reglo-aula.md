...
| `deleteAulaLesson` | action | elimina una lezione della company (mai i template; id validato Zod). Ordine: **prima la riga DB, poi il pacchetto R2 best-effort** (un pacchetto orfano su R2 è innocuo, una riga orfana senza pacchetto no). Le immagini slide NON si eliminano (namespace company condiviso). Invalida cache `AULA` |
...
| `createAulaLiveSession` | action | genera `joinCode` + `questionIds` + `mode` (`LIVE`/`EXAM`) → scrive sessione in Redis. **Priorità domande**: l'editor passa le domande dei blocchi `quizRef` della lezione (dedup); solo se la lezione non ne ha si ripiega su N casuali dal capitolo (`resolveQuestionIds` dà priorità agli id espliciti). Con modifiche non salvate l'avvio quiz **salva prima automaticamente** (se il salvataggio fallisce non si parte) |
...
- **Guard "modifiche non salvate"** (editor): tre livelli — `beforeunload` (chiusura/refresh tab), **click-capture sui link** quando `dirty` (la navigazione SPA di App Router non è bloccabile nativamente → confirm sui click `a[href]`), **autosave prima dell'avvio quiz** (che naviga via dall'editor).
- **Token sul proiettore (regola)**: l'overlay `AulaSlideShow` è una superficie dark deliberata SENZA scope `.dark` → mai usare token light del DS (`primary` = navy → invisibile su nero). Colori hardcoded light-on-dark: bianco, giallo brand `#FACC15` (bullet), `positive` ok perché verde. CTA primaria = bianco pieno, secondarie = `DARK_BTN` outline.
