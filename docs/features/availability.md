...
- **Web publication-mode editor (2026-07-07)**: la Disponibilità istruttore vive nel tab "Disponibilità" dell'hub Gestisci (`tabs/InstructorsTab.tsx`, redesign 2026-07-12; prima era un dialog in `AutoscuoleResourcesPage`) ed è mode-aware (`availabilityMode` dai settings istruttore). In modalità publication mostra `InstructorPublicationEditor` (rail 8 settimane, Pubblica/Ritira via `publishInstructorWeek`/`unpublishInstructorWeek`, righe giorno → `setDailyAvailabilityOverride`, template settimane nuove = ultima pubblicata → base, stessa catena del publish server-side) al posto dei tab Predefinito/Calendario; badge modalità + "Cambia modalità" nell'header (merge settings via `updateAutoscuolaInstructor` — ATTENZIONE: quel campo settings SOSTITUISCE il JSON). Fix incluso: `publishInstructorWeek` materializzava i giorni da `defaultAvail.ranges` piatte → con base per-giorno (`rangesByDay`) pubblicava tutti i giorni come riposo; ora usa `rangesForDay` (valeva anche per il publish mobile). `getAutoscuolaInstructorWeeklyAvailabilities` espone anche `rangesByDay`.
