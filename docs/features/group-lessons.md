...
- `updateGroupLesson({groupLessonId, startsAt?, endsAt?, instructorId?, vehicleId?})` — edits time/instructor/vehicle and **cascades to all participant appointments**. Consumed by the mobile manage modal (auto-save on instructor/vehicle change, "Sposta" for time).
