...
- → **Tipo guida moto (birilli/strada, REG-399)**: campo strutturato opzionale `AutoscuolaAppointment.motoLessonType` (`"birilli"|"strada"|null`) per le guide moto **individuali**. Si imposta in creazione (popover, modalità Moto) e in modifica (`updateAutoscuolaAppointmentDetails`); serializzato dal bootstrap agenda (`motoLessonType: true` nel select) → badge pill sul blocco + riga popover. Modulo condiviso `lib/autoscuole/moto-lesson-type.ts`. Gruppi moto fuori scope v1; mobile non lo consuma ancora. See `features/moto-lesson-type.md`.
