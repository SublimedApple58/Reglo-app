...
- → **Group Lessons (self-booking, REG-420, 2026-08-29)**: il self-enrol ai gruppi ora rispetta `bookingBlocked` (raw) — `respondGroupLessonInvite` rifiuta, `getGroupLessonInvites` → lista vuota, `broadcastGroupLessonInvite` esclude i bloccati. Prima era un buco (solo le singole guardavano il blocco). Staff (create/add) invariati. Ogni NUOVO path che fa acquisire un posto di gruppo va guardato allo stesso modo.
