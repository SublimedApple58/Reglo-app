...
### Account Consorzio
- → **Appointments**: l'accettazione di una `ConsorzioGuideRequest` crea un `AutoscuolaAppointment` normale (`bookingSource: "consortium_request"`); la Fatturazione legge guide ed esami (`type: "esame"`, voce a tariffa esame REG-459) non annullati del mese (esclusi i gruppi). Nessun punto di creazione appuntamento è stato modificato (prezzo lazy, vedi `features/consorzio.md`).
