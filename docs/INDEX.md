...
| Autoscuole consorziate come Company Reglo + inviti titolare (REG-454) | [consorzio.md](features/consorzio.md) | `lib/actions/consorzio-affiliate.actions.ts`, `lib/consorzio/affiliate-invites.ts`, `lib/consorzio/affiliate-name.ts`, `lib/service-access.ts` (`requireAffiliateSchool`), `app/[locale]/backoffice/consorzi/[companyId]`, `BackofficeConsorzioDetailPage.tsx`, `SchoolAccessCard.tsx`, `scripts/backfill-consorzio-affiliates.ts` |
| Vista ridotta della consorziata senza Reglo (REG-429) | [consorzio.md](features/consorzio.md) | `components/pages/Autoscuole/locked/*` (`AffiliateAgendaPage`, `LockedSettingsPane`, `LockedMenuItem`, `SectionLockedCards`), `lib/actions/affiliate.actions.ts`, `AutoscuoleStudentsPage.tsx` (flag `affiliate`), `AutoscuoleResourcesPage.tsx`, `lib/services.ts` (`isAffiliateWithoutReglo`), `lib/service-access.ts` (`requireAffiliateSchool`), `components/ui/feedback-toast.ts`, `public/images/locked/*` |
| REG-429 — decisioni prese senza prototipo | [reg-429-decisioni-notturne.md](features/reg-429-decisioni-notturne.md) | elenco puntuale delle scelte di dettaglio (pane bloccate, slot occupati, colonne agenda) da rivedere |
...
| Password Reset (web + mobile) | [password-reset.md](features/password-reset.md) | `lib/auth/password-reset.ts` (cuore condiviso), `lib/actions/password-reset.actions.ts`, `app/[locale]/(signin)/reset-password/*`, `lib/mobile-auth-payload.ts`, `app/api/mobile/auth/password-reset/*` |
| Pagine di accesso web (sign-in, sign-up, reset-password) | [login-web.md](features/login-web.md) | `app/[locale]/(signin)/{auth-shell,auth-form-ui,review-panel,marketing-links}` + `sign-in/`, `sign-up/`, `reset-password/`, `public/images/auth/*`, `user.actions.ts` (`signInWithCredentials`, `signUpUser`) |
...
| Tariffe consorzio: da quando vale un nuovo prezzo | [consorzio.md](features/consorzio.md) | `lib/consorzio/pricing-change.ts`, `consorzio.actions.ts` (`getConsorzioPricingChangeImpact`, `updateConsorzioPricing` con `applyTo`), `PricingApplyDialog.tsx`, `ConsorzioPrezziPane.tsx` |
| Esito esame (idoneo/respinto + numero patente) | [exam-outcome.md](features/exam-outcome.md) | `lib/autoscuole/exam-outcome.ts`, `autoscuole.actions.ts` (`setExamOutcome`), `ExamOutcomePanel.tsx`, `AutoscuoleAgendaPage.tsx`, `AutoscuoleStudentsPage.tsx`, `ConsorzioStudentDrawer.tsx`, migration `20260923150000_exam_outcome` |
| Pausa tra una guida e l'altra (REG-484) | [lesson-buffer.md](features/lesson-buffer.md) | `lib/autoscuole/lesson-buffer.ts` (`createLessonBufferBlock`, `lacksRoomForLessonBuffer`), `autoscuole.actions.ts` (`createAutoscuolaAppointment` + batch), `autoscuole-availability.actions.ts` (`createBookingRequest`, `respondWaitlistOffer`), `autoscuole-settings.actions.ts` (`lessonBufferEnabled`/`lessonBufferMinutes`), `tabs/BookingsTab.tsx`, `AutoscuoleAgendaPage.tsx` (blocco «Pausa» + conferma `LESSON_BUFFER_CONFIRM`) |
| Filtri lista Allievi (patente, cambio, luogo, istruttore — REG-469) | [students-filters.md](features/students-filters.md) | `filters/ToolbarFilters.tsx` (condiviso con l'agenda), `AutoscuoleStudentsPage.tsx` (`filteredStudents`, `filterGroups`), `AutoscuoleAgendaPage.tsx` |
| Blocco prenotazioni in bulk dalla sezione Allievi (+ scadenza "fino a data X", REG-442) | [bulk-booking-block.md](features/bulk-booking-block.md) | `lib/autoscuole/booking-block.ts` (puro) + `booking-block-expiry.ts` (sweep), `autoscuole.actions.ts` (`setStudentsBookingBlock`, `applyStudentsBookingBlock`, `toggleStudentBookingBlock`), `CompanyMember.bookingBlockUntil`, `AutoscuoleStudentsPage.tsx` (checkbox + barra flottante + dialog), `trigger/autoscuole-booking-block-expiry.ts` |
