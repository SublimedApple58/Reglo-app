...
| Tipo guida moto (birilli/strada) | [moto-lesson-type.md](features/moto-lesson-type.md) | `lib/autoscuole/moto-lesson-type.ts`, `autoscuole.actions.ts` (`AutoscuolaAppointment.motoLessonType` in create/updateDetails + bootstrap select), `AutoscuoleAgendaPage.tsx` (selettore create + badge blocco), `EditAppointmentDialog.tsx` (selettore modifica) |
