...
| Login web (`/[locale]/sign-in`, design allineato al sito marketing) | [login-web.md](features/login-web.md) | `app/[locale]/(signin)/{auth-shell,auth-form-ui,review-panel,marketing-links}`, `app/[locale]/(signin)/sign-in/{page,credentials-signin-form}`, `public/images/auth/*`, `user.actions.ts` (`signInWithCredentials`) |
