          ...
          <CardHeader className="relative flex flex-row items-start justify-between gap-3">
            <div>
              <CardTitle className="flex items-center gap-2 text-base">
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-50 shadow-inner">
            ...
            <Badge variant="outline" className="border-gray-200 bg-gray-50 text-xs">
              Beta
            </Badge>
          </CardHeader>
          <CardContent className="relative space-y-4">
            <div className="relative rounded-3xl border border-gray-200 bg-gray-50 shadow-inner">
              <div
                ref={listRef}
              ...
              >
                ...
                {!messages.length && (
                  <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-3 py-2 text-sm text-muted-foreground shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {frequentQuestions.map((question) => (
                        <motion.button
                          ...
                          className="rounded-full border border-gray-200 bg-white px-3 py-2 text-left text-sm text-foreground/80 transition hover:border-primary/40 hover:text-foreground"
                        >
                          {question}
                        </motion.button>
                      ))}
                ...
                <form onSubmit={handleSend} className="relative space-y-2">
                  <div className="relative">
                    <Textarea
                      ...
                      className="min-h-[110px] w-full border-gray-200 bg-white pr-20 text-sm shadow-sm focus-visible:ring-1 focus-visible:ring-primary/50"
                      rows={3}
                      onKeyDown={(event) => {
                  ...
                  <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                    ...
                    <Badge variant="outline" className="border-gray-200 bg-gray-50 text-[10px]">
                      Mock agent
                    </Badge>
                  </div>
          ...
          <Card className="glass-panel">
            ...
            <CardContent className="space-y-2">
              {slashCommands.map((item) => (
                <div
                  key={item.command}
                  className="flex items-start justify-between rounded-2xl border border-gray-200 bg-white px-3 py-2 shadow-sm"
                >
                  <div>
